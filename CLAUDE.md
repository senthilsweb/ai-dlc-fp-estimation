...
- **No hardcoded estimation constants — anywhere.** Every value that feeds effort or cost (base PDR, tech/productivity factors, rate, hours/day, days/month, VAF base and increment) must come from the dataset and be a live input in Effort & Cost. A numeric literal inside a formula that doesn't trace back to a dataset field is a bug. The `numOr(value, fallback)` guards exist only so a dataset that omits a field still renders; the dataset always wins.
- **Effective PDR composes, never set directly.** `effective = nominal PDR × Π(active productivity factors)`. PDR is *Project Delivery Rate* (hours per Function Point); nominal is the **human baseline before AI**, never pre-discounted, or it double-counts against the factors — which is where the AI speedup belongs. The chain is displayed so estimates stay auditable and calibration can tell which term was wrong.
- **Technology stack is documentation only.** It feeds proposal/SOW generation and must not affect hours or cost. Only nominal PDR and productivity factors drive effort.
- **Vendor-neutral factor names.** Use "AI Agent-Assisted" / "AI Agent-Led", never product names (Copilot, Claude, etc.) in dataset labels or descriptions.
- **Assumption granularity is the estimator's choice.** One config applies to all bolts in a dataset; per-bolt assumptions are done by splitting datasets and switching with `?app=`. Don't build per-bolt override machinery.
