...
- **Effective PDR composes, never set directly.** `effective = basePDR × techFactor × Π(active productivity factors)`, and `basePDR` is the *unadjusted human baseline* — never pre-discounted for AI, or it double-counts against the productivity factors. The composition chain is displayed so estimates stay auditable, and so calibration can tell which term was wrong.
